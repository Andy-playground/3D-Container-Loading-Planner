...
## 9. 修正實作紀錄（Addendum，2026-07-11 / v3.2）

本節記錄依據上述評估所完成的演算法重構（`src/packer.js`），以及新增的實務功能。

### 9.1 已修正的 Bug

| # | 問題 | 修法 |
|---|---|---|
| Bug 1 | 剩餘貨物全數無法放置時產生大量空櫃 | 多櫃迴圈中若整櫃放置數為 0 → 不納入該櫃並停止展開；`packAuto` 評分同步恢復正確 |
| Bug 2 | `maxLoadOnTopKg` 只驗直接支撐層 | 改為**支撐鏈逐層驗證**：新箱重量依「接觸面積比例」分配給各支撐者並沿支撐鏈向下傳遞，鏈上每一箱的累計承載都須 ≤ 其 `maxLoadOnTopKg`。同時修掉舊 `computeTopLoad` 在菱形支撐（一箱跨壓兩箱、兩箱同壓一箱）情境的重複累計 |
| Bug 3 | 死碼 `supporterCargo` | 隨重構移除 |

### 9.2 金字塔式疊貨（重下輕上）

1. **排序**：priority → **重量由重到輕** → 體積由大到小（原本僅體積）。重貨先裝自然佔據底層。
2. **Best-fit 評分制**：原本 first-fit 改為對所有可行 (EP × 方向) 評分取最佳，評分鍵（lexicographic）：`z 最低 → 不壓在較輕箱上（金字塔懲罰）→ x 最深 → y 最小`。z 為首鍵可保留提早中止（early break），效能不退化。
3. 實測（20GP、輕重同尺寸各 10 箱、地面僅容 10 箱）：修正前 10 箱 90kg 全數疊在 5kg 輕箱上、重心高度 159cm；修正後重箱全在底層、重心 61cm、「重壓輕」0 件。

### 9.3 不可堆疊（nonStackable）

- 新增貨物選項 `nonStackable`（UI 勾選、CSV `non_stackable` 欄、JSON 匯入正規化）。
- 語意：**僅放櫃底地面（z=0）、上方永不放置任何貨物**。適用鋰電池、輪組等打棧板貨——使用者直接輸入打棧板後的整體長寬高與總重。
- 同時修掉舊版漏洞：`maxLoadOnTopKg=0` 的箱子仍可被「零重量箱」疊上（0 ≤ 0 通過）；現在 `nonStackable` 與 `maxLoadOnTopKg≤0` 都會硬性擋掉任何上方放置。

### 9.4 效能

- `computeTopLoad` / `computeLayer` 熱迴圈遞迴移除：支撐圖（`supports` + 面積比例）、層數（`layer`）、累計承載（`carriedKg`）快取於 placement，放置時增量更新。
- 承重鏈驗證以 z 遞減拓撲序單次傳遞（O(V+E)），無路徑爆炸。
- EP 新增**向下投影**（懸空側向極點投影到下方最高實體表面），提高可放置點覆蓋率；並剔除櫃壁上的無效 EP。
- 實測：1000 箱混合尺寸 ≈ 110ms（NFR-1 上限 3s）；4 組情境對照利用率與櫃數與舊版持平或更佳。

### 9.5 測試

新增 T15–T22（合計 90 斷言）：金字塔重下輕上、nonStackable 地面限定＋頂部禁放、承重鏈超載拒絕（§4.1 Bug 2 實例）、supportRatioMin 門檻、全數無法放置不產生空櫃（鎖住 Bug 1）、pitch/roll 旋轉尺寸、maxStackLayers=2 中間值、1000 箱效能煙霧測試。

### 9.6 尚未處理（維持原建議）

- LIFO 真門邏輯（§3-3）：`priority:'lifo'` 仍為排序偏置。
- `rotationStep` 45/15°（§3-1）與棧板子貨物 `palletItems[]`（§3-2）：`nonStackable` + 打棧板整體尺寸輸入已涵蓋主要實務情境，其餘維持 future。
